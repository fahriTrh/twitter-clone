const SidebarItem = ({ type, width, height, isBlue = false, children, onClick }) => {
    return (
        <div onClick={onClick} className={`w-auto h-auto lg:w-[min-content] p-3 lg:p-2 lg:px-3 flex items-center lg:gap-3 my-1 lg:my-2 xl:my-3 rounded-full transition-all ${type == 'home' || type == 'bird' || type == 'profile' || type == 'feather' ? 'cursor-pointer' : 'cursor-not-allowed' } ${isBlue ? 'bg-sky-600 lg:w-auto hover:bg-sky-500 mt-3' : 'hover:bg-neutral-700'}`}>
            {
                type == 'home' && (
                    <>
                        <svg
...

            {
                type == 'feather' && (
                    <>
                        ...
                        <span className="hidden lg:block font-semibold text-xl mx-auto">
                            {children}
                        </span>
                    </>
                )
